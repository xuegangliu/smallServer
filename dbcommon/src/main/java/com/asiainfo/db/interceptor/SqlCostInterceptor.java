package com.asiainfo.db.interceptor;
...
/**
 * Created by xuegangliu on 2017/11/1.
 * Sql执行时间记录拦截器
 * Executor（update、query、flushStatements、commint、rollback、getTransaction、close、isClosed） 有可能从一二级缓存查找
...
//    @Value("${query.max.time}")
//    private Integer MAX_QUERY_TIME;

    private final static Logger logger = LoggerFactory.getLogger(SqlCostInterceptor.class);
...
//            logger.info("预编译sql语句:"+sql);
            ...
            // TODO sql语句执行时间
//            logger.info("过滤查询时间="+MAX_QUERY_TIME);
//            if(MAX_QUERY_TIME >= sqlCost){
            logger.info("**********SQL=[" + sql + "] ,执行时间= [" + sqlCost + "ms]");
//            }else{
//                logger.error("SQL=[" + sql + "] ,执行时间= [" + sqlCost + "ms]");
//            }
        }
    }
